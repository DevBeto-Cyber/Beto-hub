...
local S = Library:AddServer("Scripts", "http://www.roblox.com/asset/?id=16329392936")

local H = S:AddTab("Home")
local Main = S:AddTab("Character")
local T = S:AddTab("Target")
local A = S:AddTab("Animations")
local E = S:AddTab("Esp")
local Misc = S:AddTab("Misc")
local O = S:AddTab("Others")
              --//Variaveis\\--
...
local RunTime = H:AddLabel("Game Time:");
...
function UpdateTime()
    ...
    if runTimeHours < 1 then
        if runTimeMinutes < 1 then
            RunTime:Set(runTimeSeconds .. " Second(s)")
        else
            RunTime:Set(runTimeMinutes .. " Minute(s), " .. secondsRemaining .. " Second(s)")
        end
    else
        RunTime:Set(runTimeHours .. " Hour(s), " .. minutesRemaining .. " Minute(s), " .. secondsRemaining .. " Second(s)")
    end

    Time:Set(datetime .. " [ " .. code .. " ]");
end
--------------------------------------------------
              --//Character\\--
...
Main:AddToggle("Void Protection",false,function(bool)
  if bool then
   ToggleVoidProtection(true)
	else
		ToggleVoidProtection(false)
...
---------------------------------------------------                --//Target\\--
        ...
        while wait() do
            pcall(function()
                for i,v in pairs(game:GetService("Players"):GetPlayers()) do
                    if i == 81 then
                        plyserv:Set("Players :".." "..i.." ".."/".." ".."200".." ".."(Max)")
                    elseif i == 1 then
                        plyserv:Set("Player :".." "..i.." ".."/".." ".."200")
                    else
                        plyserv:Set("Players :".." "..i.." ".."/".." ".."200")
                    end
                end
            end)
        end
    ...
    local SelectedPly = T:Dropdown("Select Player",Playerslist,function(value)
        _G.SelectPly = value
    end)
...
---------------------------------------------------
                   --//Misc\\--
  Misc:AddButton("Chat Alert",function()
    for i = 1,3 do
  ...
  Misc:AddToggle("Anti Chat Spy", false, function(enabled)
    if enabled then
        while true do
            task.wait()
  ...
  Misc:AddToggle("Anti Afk", false, function(isEnable)
    if isEnable then
  ...
  Misc:AddTextbox("CmdBar", "CMD Bar...", true, function(t)
  command = t
